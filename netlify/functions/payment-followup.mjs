// Netlify Function: send a one-time payment follow-up email to clinics that
// chose the $70 Economy plan but have not completed payment.
//
// Route through netlify.toml redirect: POST /api/payment-followup
//
// Env vars:
...
function isAuthorized(req, payload) {
  const secret = process.env.CAMPAIGN_ADMIN_SECRET
...
async function markFollowupSent(clinic, sentAt) {
  const nextData = { ...(clinic.data || {}), paymentFollowupSentAt: sentAt }
  await supabaseJson(`clinics?id=eq.${encodeURIComponent(clinic.id)}`, {
    method: 'PATCH',
...
export default async (req) => {
  ...
  if (!isAuthorized(req, payload || {})) return json({ ok: false, error: 'unauthorized' }, 401)
  ...
  const from = process.env.RESEND_FROM || `DentalCloud <${supportEmail}>`
  const siteUrl = process.env.SITE_URL || ''

  try {
    const clinics = await supabaseJson('clinics?select=id,owner_id,data,created_at')
    const doctors = await supabaseJson('doctors?select=id,clinic_id,data')
    ...
    for (const clinic of candidates) {
      ...
      try {
        ...
        await markFollowupSent(clinic, sentAt)
